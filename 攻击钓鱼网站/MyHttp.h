...
public:
	...
	~MyHttp();
	//bool PageAccess(std::string pageAddr, std::ofstream &fileObject, std::string fileFormat);
	std::vector<char>& GetHtmlByte();
	std::string PageAccess(std::string pageAddr, std::string accessMethod, std::string data );
	...
	bool DownImage(std::string url, std::string filename);
...
std::string MyHttp::PageAccess( std::string pageAddr, std::string accessMethod, std::string data = "")
{
...
bool MyHttp::DownImage(std::string url, std::string filename)
{
	
	//此代码会与windouws标准库冲突！
	/*
	int nmlen = MultiByteToWideChar(CP_ACP, 0, url.c_str(), url.length() + 1, NULL, 0);
	wchar_t* buffer = new wchar_t[nmlen];
	MultiByteToWideChar(CP_ACP, 0, url.c_str(), url.length() + 1, buffer, nmlen);
	CString str = CString(filename.c_str());
	USES_CONVERSION;
	LPCWSTR wszClassName = A2CW(W2A(str));
	str.ReleaseBuffer();
	if (URLDownloadToFile(NULL, buffer,wszClassName, 0, NULL)==S_OK)
	{
		delete[] buffer;
		return true;
	}
	...
	//相同效果！
	/*std::vector<char> vec = this->GetHtmlByte();
	if (vec.size() <= 0)
	{
		return false;
	}
	...
	*/
	...
	std::string html = this->GetHtml();
	if (html.size() != 0)
	{
		std::ofstream file(filename, std::ios::binary);
		for (std::string::iterator it = html.begin(); it != html.end(); ++it)
		{
...
//废弃的网页访问
/*bool MyHttp::PageAccess(std::string pageAddr, std::ofstream &fileObject, std::string fileFormat)
{

	std::stringstream request;
	request << "GET " + pageAddr << "\r\n";
	request << "\r\n";
	std::string Get = request.str();
	std::cout << Get << std::endl;
	if (send(this->Socket, Get.c_str(), Get.size(), 0) > 0)
...
std::vector<char> & MyHttp::GetHtmlByte()
{
	char buf[1024];
	memset(&buf, 0, sizeof(buf));
	int n = 0;
